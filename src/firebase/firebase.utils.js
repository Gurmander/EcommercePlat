...
// Stores userauth in database
export const createUserProfileDocument = async (userAuth, additionalData) => {
  if(!userAuth) return;

  // Random collection/id-reference type
  // console.log(firestore.doc('users/1231fds'))
  // ;
  const userRef = firestore.doc(`users/${userAuth.uid}`); 
  // userAuth.uid is unique id that's generated by firebase during auth process under the hood
  const snapShot = await userRef.get();

  // console.log(snapShot);
  // snapShot further contains exists property, which basically tells whether this particular data exists or not in firestore database

  // if snapshot doesn't exist, set the user in database 
  if(!snapShot.exists) {
